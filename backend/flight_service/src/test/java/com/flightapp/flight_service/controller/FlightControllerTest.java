...
public class FlightControllerTest {
    ...
    public void testSearchFlights_Success() throws Exception {
        doReturn(List.of(new FlightResponse())).when(flightService).searchFlights(any());
    ...
    public void testAddFlight_Failure() throws Exception {
        ...
        mockMvc.perform(post("/api/v1.0/flight")
                .contentType("application/json")
                .content("""
                {
                  "airlineName":"TestAir",
                ...
                .andExpect(jsonPath("$.message").value("Failed to add flight"));
    }
    @Test
    public void testUpdateFlightStatus_Success() throws Exception {
        mockMvc.perform(post("/api/v1.0/flight/1/status?status=AVAILABLE"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").value("Flight status updated successfully"));
    }
    @Test
    public void testBlockAirline_Failure() throws Exception {
        ...
        mockMvc.perform(post("/api/v1.0/flight/airline/99/block"))
                .andExpect(status().isInternalServerError())
    ...
    public void testAddFlight_Success() throws Exception {
        ...
        mockMvc.perform(post("/api/v1.0/flight")
                ...
                """))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.airlineName").value("TestAir"));
    }
    @Test
    public void testUpdateFlightStatus_Failure() throws Exception {
        ...
        mockMvc.perform(post("/api/v1.0/flight/1/status?status=CANCELLED"))
                .andExpect(status().isInternalServerError())
                .andExpect(jsonPath("$.message").value("Flight not found"));
    }
    @Test
    public void testSearchFlights_NoResults() throws Exception {
        doReturn(List.of()).when(flightService).searchFlights(any());
    ...
    public void testBlockAirline_Success() throws Exception {
        mockMvc.perform(post("/api/v1.0/flight/airline/1/block"))
                .andExpect(status().isOk())
    ...
    public void testRegisterAirline_Failure() throws Exception {
        ...
        mockMvc.perform(post("/api/v1.0/flight/airline/register")
                ...
                """))
                .andExpect(status().isInternalServerError())
                .andExpect(jsonPath("$.message").value("Failed to register airline"));
    }
}